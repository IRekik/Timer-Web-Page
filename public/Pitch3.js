...
var Pitch = {
  ...
  updateText: function(e) {
    if (e !== undefined) {
        Pitch.progressText.html(e);
    } else if (Pitch.progress === 1) {
        Pitch.progressText.html("Time Expired");
    } else {
        Pitch.progressText.html("Start");
    }
  },
  onTimeComplete: function () {
    Pitch.updateText("Time Expired");
    if (Pitch.progress === 1) {
        Pitch.beep && Pitch.beep.play && (Pitch.beep.volume = Pitch.volume, Pitch.beep.play());
        clearInterval(Pitch.ticker);
        Pitch.updateText("Time Expired");
        Pitch.showAlert();
    } else if (Pitch.sequence.length > 0) {
        var e = Pitch.sequence.shift();
        Pitch.initializeTimer(0, 1e3 * (e.duration || 0));
    }
  },
  showAlert: function() {
    Pitch.canAlert;
  ...
  getTimeRemaining: function() {
      ...
      var remainingTime = Pitch.endTime - currentTime;
...
function setInitialValues () {
  ...
  Pitch.startTime = Date.now();
  Pitch.endTime = Date.now() + (2 * 60 * 1000);
  Pitch.parseError = "none";
  Pitch.volume = 1;
  Pitch.canAlert = false;
}